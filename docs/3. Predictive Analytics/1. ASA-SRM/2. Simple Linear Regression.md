...
!!! Warning
    ...
    The interpretation of $\beta_{0}$ is that it is $E(Y \mid X = 0)$. However, the coefficient is NOT $\bar{y} \mid x = 0$ for that sample. This is expected, because the fitted model is based on a sammple, some discrepancy is expected:
...
!!! Warning
    ...
    * Squareroot of Mean Square of Residuals is sometimes known as the **Standard Error of Regression**
    * Mean Square of Total is the **unbiased sample variance**
