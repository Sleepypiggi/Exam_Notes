...
!!! Info
    ...
    The parameters are also sometimes collectively referred to as the **Regression Coefficients**.
...
Following traditional statistical inference, it is important to assess the quality of the fitted parameters. Thus, it is necessary to understand their **sampling distribution**.

Recall the expression for the SLR parameters:
...
!!! Warning
    ...
    Note that the the Variance of the prediction, the numerator uses $x$ rather than $x_{i}$. This is to reflect that it should be using the **value of $x$ for that specific prediction**.
...
For prediction intervals, the SE of the prediction error must be first determined:

$$
\begin{aligned}
    \text{Var}(y_{i} - \hat{y}_{i})
    &= \text{Var}(y_{i}) + \text{Var}(\hat{y}_{i}) \\
    &= \sigma^{2}_{\text{RSS}} + \sigma^{2}_{\text{RSS}} \cdot \left(\frac{1}{n} + \frac{(x - \bar{x})^{2}}{\sum (x_{i} - \bar{x})^{2}} \right) \\
    &= \sigma^{2}_{\text{RSS}} \cdot \left(1 + \frac{1}{n} + \frac{(x - \bar{x})^{2}}{\sum (x_{i} - \bar{x})^{2}} \right)
\end{aligned}
$$

Thus, the prediction interval can be constructed using the general expression:

$$
    \text{PI} = \hat{\theta} + t_{\frac{1-\alpha}{2}, n-2} \cdot \text{SE}(y_{i} - \hat{y}_{i})
$$
