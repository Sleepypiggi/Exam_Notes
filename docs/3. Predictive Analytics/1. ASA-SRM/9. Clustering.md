...
!!! Tip
    ...
    It can be shown that the Cluster Variation can also be calculated as **TWO times** the Euclidean Distance from each observation to its centroid:

    $$
        \text{Total Cluster Variation} = 2 \sum \text{ED from Centroid}
    $$
