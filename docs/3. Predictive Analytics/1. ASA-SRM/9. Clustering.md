...
Clustering is an **Unsupervised** statistical learning method that is used to find **Clusters** within the dataset. They are a **subgroup of observations** that are **similar** to one another; different clusters naturally have different characteristics.
...
* **Observation**: Grouping Customers into Income brackets
...
Due to the iterative nature of the method, a *hierarchy* of clusters are formed, where clusters are **subsets of one another**. The clustering can be **"sliced"** to determine the final number of clusters. It is thus best visualized in a tree-like structure known as a **Dendrogram**:
...
The choice of linkage does affect how the **shape** of the resulting Dendrogram:

* **Single**: Tends to merge single observation clusters first, resulting in a **skewed diagram**
