...
Due to the linear algebra required, the data must first be **Centered** and **Scaled** (normalization). Each PC then **passes through the mean** of the data (which is now the origin post centering):
...
!!! Note
    ...
    The reason for scaling is that variables with larger scale will naturally have higher variance, thus skewing the results.
...
!!! Note
    ...
    In general, the PC that explains the most variance among the predictors (first PC) is not guaranteed to best explain the target variable.