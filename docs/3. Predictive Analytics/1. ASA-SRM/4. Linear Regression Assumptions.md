...
Naturally, these observations have **large residuals**. However, residuals contain the units the of the dependent variable, which affects its scale. Thus, the residuals must be transformed into a **unitless form** to determine a universal benchmark.
