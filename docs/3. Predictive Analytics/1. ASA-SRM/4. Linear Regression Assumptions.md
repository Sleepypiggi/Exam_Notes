...
For large samples, CLT states that the distribution of the estimators are **approximately normal**, thus inferences are still valid.

However, this does NOT apply to prediction intervals as they directly contain the variance of the error term, which follows an unknown distribution.
