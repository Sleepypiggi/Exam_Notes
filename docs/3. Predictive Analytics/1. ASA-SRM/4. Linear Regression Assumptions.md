...
For large samples, CLT states that the distribution of the estimators are **approximately normal**, thus inferences are still valid. However, this does NOT apply to prediction intervals as they directly contain the variance of the error term, which follows an unknown distribution.

## **Other Considerations**

### **Leverage**

Observations with unusually high values in the Independent Variable are known as **High Leverage Points**.

There is **nothing inherently wrong** with observations with high leverage - they could be representing a subgroup within the population with different characteristics.

The issue comes when the model is mainly driven by a few high leverage points such that the model would be **vastly different without them**, creating instability.

The easiest way to identify high leverage points would be through a scatterplot of independent variables:

<!— Obtained from ACTEX Manual —>
![High Leverage Points](Assets/4.%20Gauss%20Markov%20Theorem.md/High%20Leverage.png){.center}

When there are multiple variables, another consideration is that the observation might seem normal within each variable, but **unusual when taken collectively**. This is hard to visualize, thus the leverage can also 
...
!!! Note
    ...
    Range of h, total leverage and intuition
...
Observations with unusually high values in the dependent variable are known as **Outliers**.
...
Outliers are usually associated with **miscoded or missing observations**, but they could also be due to unmodelled behaviour for a sub-group of predictors. Investigation needs to be done to understand what is driving the outliers, and hence whether or not they can be omitted.

The issue is that **OLS is sensitive** to outliers as it uses the residuals as the basis to estimate the the coefficients; a model with and without outliers can have very different estimates:

<!-- Obtained from Quantitative Research Methods for Political Science -->
![OUTLIERS](Assets/4.%20Linear%20Regression%20Assumptions.md/OUTLIERS.png){.center}
