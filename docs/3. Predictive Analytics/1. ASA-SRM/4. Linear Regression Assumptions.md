...
As alluded to previously, increasing the number of parameters in the model does NOT necessarily benefit the model. In fact, it can be detrimental via a Phenomenon known as the **Curse of Dimensionality**.
...
!!! Warning
    ...
    To be precise, only the first PLS direction is a direct combination of the underlying predictors.
    
    Subsequent directions use a **combination of the residuals** of a regression using previous directions to predict the the **original predictors**
    
    The idea sounds *similar* to Boosting but is not exactly it as well.

The key difference when compared to PCA is that PLS finds new directions with the **inherent goal of explaining the dependent variable**. Consequently, cross validation can be used to determine the optimal number of directions.
