...
$$
\begin{aligned}
    E(y_{t}) &= y_{0} + \bar{\varepsilon} \\
    \text{Var}(y_{t}) &= t \cdot \sigma^{2} \\
    \\
    E(y_{t} - y_{t-1}) &= \bar{\varepsilon} \\
...
!!! Warning
    ...
    If Conditional Least Squares is used, then the second term reduces to 0 as a result of the minimization. However, if the approximation is used, the second term DOES NOT reduce to 0, thus it needs to be calculated explicitly.
...
\begin{aligned}
    ...
    &= (1-w) y_{t} + w (1-w) \cdot y_{t-1} + \dots + w^{t} \cdot y_{0}
\end{aligned}
$$
...
!!! Tip
    ...
    $w$ is the weight placed on HISTORICAL observations.
    ...
    A **typical value** for $w$ is within the range $(0.75, 0.95)$.
...
!!! Tip
    ...
    Naturally, this can be used to determine the **in-sample goodness of fit** as well.
...
!!! Warning
    ...
    The ultimate functional form is the same as the moving average smoothing. However, the individual estimates for $\beta$ are different. This is especially true for $\beta_{0}$, where the estimate *looks the same* but is **actually different**. 
