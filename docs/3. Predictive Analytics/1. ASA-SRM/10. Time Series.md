...
!!! Note
    ...
    The effect of smoothing is **proportional** to $w$. Note that higher weights are placed recent observations while lower weights are placed on older observations. A **typical value** for $w$ is within the range $(0.75, 0.95)$.

    Note that $w$ is the weight placed on HISTORICAL observations.
