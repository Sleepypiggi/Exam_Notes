...
The main purpose for expressing it in this way is to perform a **Unit Root Test** - which tests if the differenced series follows a Random Walk:
...
!!! Warning
    ...
    Unlike other statistical tests, the desired outcome is the Null, not the alternative. This is known as a Weak  Hypothesis test.
...
!!! Tip
    ...
    A similar process can be done for moving average smoothing as well to determine the optimal $k$.
