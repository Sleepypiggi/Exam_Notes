...
!!! Tip
    ...
    However, a common variation of the above is to use a **Proportional Odds Cumulative Model**. It assumes that the **odds ratios are the same across thresholds**, thus allowing each equation to use the **SAME $\beta$**, with only the $\alpha$ changing for each, reducing the number of cofficients needed. HOWEVER, this does NOT mean that $\beta$ is not needed, it is simply constant.
...
!!! Note
    ...
    Consider the following example for different subgroups:
...
Alternatively, it is possible to create a new distribution using a **mixture** of distributions to achieve the desired property. A **heterogeneity model** is a special model that allows for **continuous mixtures**:
...
!!! Warning
    ...
    A mixture of two poissons is NOT poisson.
...
Thus, combining the Poisson and the Logistic Regression output, an adjusted PMF can be constructed:
...
In this case, there is a clear reason for the zeroes, thus the probability for the zero count should be **modelled seperate** for the non-zero values. This can be similar identified via Logistic Regression, by determining the **probability of crossing the threshold**.
