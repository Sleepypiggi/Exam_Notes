...
!!! Tip
    ...
    Similar to before, the model only provides $k-1$ equations for $k$ probabilities to be solved for. However, unlike before, this is because the cumulative probability for the last possible category is not modelled.

    Thus, the same approach can be taken to solve for the final probability:
