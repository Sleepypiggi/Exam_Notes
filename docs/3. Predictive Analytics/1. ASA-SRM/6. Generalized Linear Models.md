...
!!! Tip
    ...
    However, since $\varphi$ is **typically assumed to be 1**, the two are identical.
...
One method to overcome this is to multiply a constant ($\delta$) to the variance to **re-scale** the variance to an appropriate level:
...
For a variable with more than two levels and an order to them, a **Proportional Odds Cumulative Model** is used. It models the log of the **cumulative odds** of a category.
