...
* **Slope Parameter** ($\beta_{p}$): **Change** in **Expected Value** of $Y$ given a **one unit increase** in $X_{p}$, assuming all other X's are constant 
...
XB is known as the design matrix

!!! Note

    Matrices are defined using a **"(Row x Column)"** notation and are differentiated from scalar quantities with a **Bolded** symbol.

    **Addition and subtraction** of matrices work intuitively, with corresponding elements being added and subtracted with each other.

    **Multiplication** of matrices involves an **inverted 7**:
...
$$
    \hat{y}_{i}
    = \hat{\beta}_{0} + \hat{\beta}_{1} \cdot x_{i ,1}
    + ... + \hat{\beta}_{p} \cdot x_{i,p}
$$
...
Recall that for parametric learning methods, increasing the number of parameters tends to increase the flexibility of the model; ability to fit the data (RegSS). Thus, in a MLR setting, R-squared can be **artifically inflated** by adding more parameters, even when they are nonsensical!
...
However, rather than testing if an individual independent variable is useful, it tests if ALL independent variables are **collectively useful** in explaining the dependent:
