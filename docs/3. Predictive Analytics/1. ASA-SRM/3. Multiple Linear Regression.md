...
An **Added Variable Plot** uses plots the **residuals** from the following two models against each other:

* **A**: Independent variable against all OTHER variables
* **B**: Independent variable against ONLY the variable of interest

The correlation in the added variable IS the partial correlation coefficient. This is because the residuals from (A) **already accounts for the effects of the other variables** - it is a measure of how much of the remaining difference is explained by the new variable; measure of its predictive power.
...
\begin{aligned}
    ...
    \\
    \boldsymbol{H}
    &= \boldsymbol{X}_{i} \cdot (\boldsymbol{X}^{T} \cdot \boldsymbol{X})^{-1} \cdot \boldsymbol{X}^{T}
\end{aligned}
$$
...
!!! Tip
    ...
    Adjusted R-squared is R-squared **with a penalty applied** for the number of predictors. For a model with one predictor, both are the same.
...
!!! Tip
    ...
    There are questions which may ask for the impact of a variable on the response given some or all other variables are held constant.
    
    As shown above, the impact of a variable changes in the presence of an interaction term - which is the motivation of these questions.
    
    To know the definitive impact of the original variable on the response, consider the two scenarios with and without the interaction. Exclude variables that are held constant.
    
    If there are variables that are not held constant, consider the extreme scenarios - under those combinations, is the impact definitively in one direction?
...
!!! Tip
    ...
    A non-graphical approach to validate the Ridge (and Lasso) would be to test if the resulting model fulfils the above penalty by **manually adding** the resulting RSS and coefficients.
    
    This can also be used to determine the final ridge/LASSO model, which should have the lowest value of all possible combinations.
...
!!! Note
    ...
    The budget and shrinkage parameter are **inversely related**:
...
Before using the two methods, the predictors might need to be first **transformed**. This is because each predictor is of a **different scale** (different units), thus they need to be on the **same basis** to ensure proper results, since a penalty is placed on the magnitude of the coefficient.
...
The two regularizations methods are **Scale Equivariant** as they will provide the **same estimate** regardless of whether the underlying variables have been scaled or not. This is **NOT true for regular OLS**.
