...
!!! Note
    ...
    The above intuition applies for SLR as well. However, since there is only one variable, the F-test is akin to testing the significance of just that variable.

!!! Tip

    Similar to the adjusted R-squared, the partial F-test measures whether the reduction in RSS is "worth" the loss of the degrees of freedom to estimate the new parameters.
...
The last category that does not have its own variable is known as the **Baseline** of the model, which is the default category when **all other variables are 0**. Thus, the intepretation of the coefficients is as follows:

* $\beta_{0}$: Value of $E(Y \mid X)$ at the Baseline
* $\beta_{1}$: Change in value of $Y$ from the Baseline to chosen Category

!!! Warning

    Given the above, the choice of which category to set as the baseline matters as it will result in a different model altogether.
...
!!! Tip
    ...
    This is why LASSO can also be used as a feature selection method.

There is no clear advantage between the two; the key difference lies in the ability to drop predictors:

* **Ridge**: Will always retain variables thus allowing it to capture predictors that truly of small coefficients; at the cost of interpretability due to large number of predictors
* **LASSO**: Will always drop varaibles thus increasing the interpretability of the model; at the cost of missing predctors that truly have small coefficients
...
Before using the two methods, the predictors might need to be first **transformed**. This is because each predictor is of a **different scale**, thus they need to be on the **same basis** to ensure proper results, since they are now combined together in the constraint.
