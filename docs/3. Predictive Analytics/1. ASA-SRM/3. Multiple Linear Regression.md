...
!!! Note
    ...
    The Ridge coefficients are the point where the set of coefficients (Contours) that minimize the constrained RSS also fulfil the $\ell_{2}$ constraint; where the contours meet the Circle:
...
!!! Tip
    ...
    A non-graphical approach to validate the Ridge (and Lasso) would be to test if the resulting model fulfils the above constraint by **manually adding** the resulting RSS and coefficients.
...
!!! Note
    ...
    The LASSO coefficients are the point where the set of coefficients that minimize the constrained RSS also fulfil the $\ell_{1}$ constraint; where the contours meet the diamond:
