...
Many commonly studied sampling distributions can be shown to be normally distributed. Due to the shifting and scaling properties of the normal distribution, any normal distribution can be converted to a **Standard Normal Distribution** via a process known as **Standardization**:
...
The SE of the sampling distributor is typically a **function of the underlying** population standard deviation. However, in many cases, it is an **unknown value**. Thus, it can be estimated using the sample standard deviation instead:
...
$$
    t = \frac{\hat{\theta} - E(\hat{\theta})}{\hat{\text{SE}}(\hat{\theta})}   
$$
