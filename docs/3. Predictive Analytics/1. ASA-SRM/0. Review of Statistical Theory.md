...
The first most important set of parameters & statistics involve **Central Tendency**, which is a measure of the **most representative value** from a population or sample.
...
\hline
    ...
    \text{Sample Mean} \\
\hline
    \mu_{x} = E(X) = \sum x_{i} \cdot P(x_{i}) &
...
!!! Note
    ...
    The two are best applied to measurements that are:
...
Another important set of parameters and statistics are concerned with the **Dispersion**, which is a measure of how stretched or squeezed
...
\hline
    ...
    \text{Sample Variance} \\
\hline
    \sigma^{2} = \text{Var}(X) = \sum (x_{i} - \mu)^{2} \cdot P(x_{i}) &
    s^{2} = \frac{1}{n-1} \sum (x_{i} - \bar{x})^{2} \\
\hline
\end{array}
$$

However, the units of the variance are squared, which makes it **unintuitive**. Thus, the **Standard Deviation** which is the square root of the variance is also considered:

!!! Tip 
    
    The Variance still has the following key properties which makes it desirable: 
    
    * Values are squared thus **outliers are emphasised**, giving **better sense** of spread
    * Values are squared thus has **positive** values only, ensuring **no offsetting** effects
    * **Mathematical properties** which makes it easier to manipulate (EG. Variance of sum of independent variables is the sum of the individual variances)

    Thus, between Variance and Standard Deviation, one is not strictly better than the other. 
...
Variance is independent of Bias; an estimator can be unbiased but yet have high variance. On average, the estimator is correct, but the individual estimates are too high or too low (offsetting in the average).

Consider the common example of Archery:


The Standard Deviation of the Estimator (Sampling Deviation) is similarly the square root of the sampling variance:
...
The combination of the Bias and Variance in an estimator is known as the Mean Squared Error (MSE):

It can be shown that the MSE is also mathematically equivalent to the spread of estimates about the parameter:


!!! Warning

    Variance and MSE look functionally the same, but the key difference is what the spread is centered on:
    
    * Variance: Centered on the average of the estimates
    * MSE: Centered on the average of the parameter
