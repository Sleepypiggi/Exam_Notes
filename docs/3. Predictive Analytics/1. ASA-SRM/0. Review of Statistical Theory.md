...
!!! Tip 
    ...
    * Values are squared thus has positive spread only, ensuring no offsetting effects
    * Mathematical properties (EG. Variance of sum of independent variables is the sum of the individual variances)

    Thus, between the both, one is not strictly better than the other. 
...
\hline
    \sigma_{x, y} = \text{Cov}(X) = \mu_{x, y} - \mu_{x} \cdot \mu_{y} &
...
\hline
    \rho = \text{Corr}(X) = \frac{\sigma_{x, y}}{\sigma_{x} \cdot \sigma_{y}} &
...
An estimator is biased if and only if the Bias is 0. It can be understood that - on average, the estimator produces an estimate that is equal to the true value; individual estimates may still have errors.
