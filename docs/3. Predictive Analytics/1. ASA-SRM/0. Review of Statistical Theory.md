...
!!! Info
    ...
    The division by $n-1$ rather than $n$ in the sample variance is known as **Bessel's Correction**, to ensure that the resulting sample variance is an **Unbiased estimator** of the population variance. It is not necessary to know the proof for the purposes of this exam.
...
Degrees of freedom (df) refer to the **number of independent pieces of data that are free to vary** in the calculation of a statistic **without violating any of the constraints** of said statistic.
...
$$
    \text{df} = n - \text{Constraints}
$$

For instance, the **sample mean has no constraints**, thus the df of the mean is simply the sample size.

However, the sample variance has one constraint 
...
$$
    \sum^{n}_{1} (\theta_{i} - \bar{\theta}) \sim \sigma^{2} \cdot \chi^{2}_{n-1}   
$$  
...
\begin{aligned}
    \sum^{n}_{1} (\theta_{i} - \bar{\theta}) &\sim \sigma^{2} \cdot \chi^{2}_{n-1} \\
    \frac{\sum^{n}_{1} (\theta_{i} - \bar{\theta})}{n-1} &\sim \frac{\sigma^{2} \cdot \chi^{2}_{n-1}}{n-1} \\
...
\begin{aligned}
    ...
    &= \frac{\theta - E(\hat{\theta})}{\sqrt{\text{Var}(\hat{\theta})}} \cdot \sqrt{\frac{\text{Var}(\hat{\theta})}{s^{2}}} \\
    &= \frac{\theta - E(\hat{\theta})}{\sqrt{\text{Var}(\hat{\theta})}} \div \sqrt{\frac{s^{2}}{\text{Var}(\hat{\theta})}} \\
    &= \frac{Z}{\sqrt{\frac{\chi^{2}_{n-1}}{n-1}}}
\end{aligned}
$$
...
!!! Tip
    ...
    $$
    \begin{aligned}
        \lim_{n \to \infty} \frac{\chi_{n}}{n} \to 1 \\
        \therefore \lim_{n \to \infty} t_{n} \to Z
    \end{aligned}
    $$
...
\begin{aligned}
    (t_{n-1})^{2}
    &= \left(\frac{Z}{\sqrt{\frac{\chi^{2}_{n-1}}{n-1}}} \right)^{2} \\
    &= \frac{Z^{2}}{\frac{\chi^{2}_{n-1}}{n-1}} \\
    &= \frac{\frac{\chi^{2}_{1}}{1}}{\frac{\chi^{2}_{n-1}}{n-1}} \\
    &= F_{1,n-1} \\
    \therefore t^{2} &\sim F_{1, n-1} \\
    \therefore t^{-2} &\sim F_{n-1, 1} \\
    \\
    \lim_{n \to \infty} \frac{\chi_{n}}{n} &\to 1 \\
    \lim_{n \to \infty} F_{m, n} &\to \frac{\chi_{m}}{m} \\
    \therefore F_{m, \infty} &\sim F_{n-1, 1} \\
\end{aligned}
$$
