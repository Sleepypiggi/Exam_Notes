...
!!! Note
    ...
    The learning rate is also referred to as the **Shrinkage Parameter** as it reduces the influence of each tree on the overall prediction. Typically, a **slow learning rate** is preferred ($\lambda \lt 1$). Consequently, a lagre number of trees are required to achieve effective results.
