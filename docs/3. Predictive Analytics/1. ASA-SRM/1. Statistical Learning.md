...
Prediction is largely related to the **Flexibility** (or **Complexity**) of the model. As will be shown later, a **moderate amount of flexibility** is ideal with regards to prediction.
...
!!! Tip
    ...
    The inversion of test MSE depends largely on the underlying data generating process. **More complicated processes will benefit more from the bias reduction**, and thus may invert later along the curve, but will still eventually invert.
