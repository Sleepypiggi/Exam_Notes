...
Another consideration is the **Robustness** of the model  - the **stability or sensitivity** of the results. While some variation is expected from sample to sample, a good model will be able to **extract the underlying signal** which should be similar regardless of the sample.
