...
!!! Tip
    ...
    These questions typically require multiple iterations of the equation to a point where one of the values are **known** - either through the above **boundary conditions** or a **value provided** by the question.
...
The key idea is that at the valuation time, the life is known to be in state $i$. However, in $n$ years time, the insured **could be in any state**, INCLUDING the target state. Thus, these possibilities must be accounted for.
...
!!! Warning
    ...
    It is a common mistake to forget the probability of staying in the same state, as most state diagrams do not typically include.
...
\begin{aligned}
    \frac{d}{dt} {}_{t}V^{i}
    &= \delta_{t} \cdot {}_{t}V^{i} - \underbrace{\left(B_{t}^{i} + e_{t}^{i} - P_{t}^{i} \right)}_{B}
...
* **Released** to pay off expected benefits (while in state & upon transition)
