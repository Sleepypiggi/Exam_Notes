...
!!! Note
    ...
    The history of the process refers to **which states** the entity has been in and for **how long**. 

    The implication is that an entity who **JUST transitioned** into the state will have the **SAME probability** as another who has **been in the state for a long while**.

    While this simplifies calculations, it might not hold true as the time spent in the state might have a bearing on the probabilities of transition, especially for **mortality and morbidity**.
