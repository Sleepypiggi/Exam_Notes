...
Reserves for a person state i at time j

Reserves for a continuous assurance or annuity can be calculated using **Thiele's Differential Equation**:

Similar to before, the equation is trying to approximate the change in the reserve at that time:

* Reserve grows at the force of interest at that time
* Reserve changes in that period based on the **rate** of net cashflows (Ben + Exp  - Prem)
* Upon transitioning, reserve is released if a benefit is payable
* Upon transitioning, the reserve changes from state i to state j

Solve using Euler method as well
