...
!!! Tip
    ...
    The key to remember this is that the force of **transition** represents a transition - thus the approximation is for the transition probability. The probability of staying is the complement.
...
\begin{aligned}
    ...
    \ddot{a}^{00}_{x} + \ddot{a}^{01}_{x} + \ddot{a}^{02}_{x} &= \ddot{a}_{\enclose{actuarial}{\infty}} \\
\end{aligned}
$$
...
\begin{aligned}
    \frac{d}{dt} {}_{t}V^{i}
    &= \delta_{t} \cdot {}_{t}V^{i} + P - B - \sum_{j \ne i} \mu^{ij}_{x}(t) \cdot (b^{ij} + {}_{t}V^{j} - {}_{t}V^{i})
    \\
    ...
    {}_{t}V^{i}
    &= {}_{t-h}V^{i} + h \cdot
    \left(\delta_{t} \cdot {}_{t}V^{i} + P - B
