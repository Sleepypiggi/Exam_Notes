...
The same principles from before apply - the annual salary is the sum of the salaries received in the year. In a continuous scenario, the salaries are received every instant. To sum across all instances, integration must be used:
...
As stated previously, it is the employer's duty to ensure that they have sufficient funds to cover the DB pension liability. Thus, employers hols a **reserve** equal to the accrued liability of the pension at the time of valuation.
...
$$
\begin{aligned}
    \text{AL}_{t} + C_{t} &= \text{AL}_{t+1} \cdot vp_{x} \\
    C_{t} = \text{AL}_{t+1} \cdot vp_{x} - \text{AL}_{t}
\end{aligned}
$$
...
Given that there is only **one key difference**, the $\text{AL}_{t+1}$ can be expressed as a function of $\text{AL}_{t}$:

$$
\begin{aligned}
    vp_{x} \cdot \text{AL}_{t+1}
    ...
    \\
    \therefore C_{t}
    &= \text{AL}_{t+1} \cdot vp_{x} - \text{AL}_{t} \\
    &= \frac{n+1}{n} \cdot \text{AL}_{t} - \text{AL}_{t} \\
    &= \left(\frac{n+1}{n} - 1 \right) \cdot \text{AL}_{t} \\
    &= \frac{1}{n} \cdot \text{AL}_{t}
\end{aligned}
$$
...
\begin{aligned}
    ...
    &= (1+g) \cdot \frac{n+1}{n} \cdot \text{AL}_{t} \\
    \\
    \text{AB}_{t+1}
    ...
    \therefore C_{t}
    &= \text{AL}_{t+1} \cdot vp_{x} - \text{AL}_{t} \\
    &= (1+g) \cdot \frac{n+1}{n} \cdot \text{AL}_{t} - \text{AL}_{t} \\
    &= \left((1+g) \cdot \frac{n+1}{n} - 1 \right) \cdot \text{AL}_{t} \\
\end{aligned}
$$
...
$$
    \text{AL}_{t} + C_{t} = vp_{x} \cdot \text{AL}_{t+1} + v^{0.5} {}_{0.5}r_{x} \cdot \text{EPV Mid Year Retirements}
$$
...
$$
    \text{AVTHB}_{y} = \text{EPV}_{65} \cdot v^{65-y} \cdot \frac{r_{65}}{\ell_{y}}
$$
...
$$
    \text{Accrued Liability} = \frac{\text{Years Worked}}{\text{Total Years to Retirement}} \cdot \text{AVTHB}_{y}
$$

!!! Note

    By default, health benefits are assumed to only accrued to the **earliest possible retirement age**. This simplifies the problem, as there is **only ONE time point** to worry about.

    However, some questions my explicitly allow for linear accruals to **each possible retirement age**, thus the usual pension approach must be taken.

### **Health Funding**

Similar to pensions, the employer holds a reserve equal to the **Accrued Liability** of the plan. The contributions are determined using the same approach:

$$
\begin{aligned}
    vp_{x} \cdot \text{AL}_{t+1}
    &= vp_{x} \frac{n+1}{n+k} \cdot v^{65-(y+1)} \frac{r_{65}}{\ell_{y+1}} \cdot P_{65} \cdot \ddot{a}_{65 \mid i^{*}} \\
    &= \frac{n+1}{n+k} \cdot v^{65-y} \frac{r_{65}}{\ell_{y}} \cdot P_{65} \cdot \ddot{a}_{65 \mid i^{*}} \\
    &= \frac{n+1}{n} \cdot \frac{n}{n+k} \cdot v^{65-y} \frac{r_{65}}{\ell_{y}} \cdot P_{65} \cdot \ddot{a}_{65 \mid i^{*}} \\
    &= \frac{n+1}{n} \cdot \text{AL}_{t+1} \\
    \\
    \therefore C_{t}
    &= \text{AL}_{t+1} \cdot vp_{x} - \text{AL}_{t} \\
    &= \frac{n+1}{n} \cdot \text{AL}_{t} - \text{AL}_{t} \\
...
!!! Note
    ...
    Notice that this is the **exact same result** as the pension contribution under a PUC basis - both are essentially just accounting for an additional year of accruals.
