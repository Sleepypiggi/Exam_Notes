...
The goal is to determine the **expected benefit** that the employee will receive given the contributions. This is done by solving for the **Yearly Retirement Benefit** based on the *accumulated* fund value:

$$
    \text{Accumulated Fund Value} = \text{Yearly Retirement Benefit} \cdot \ddot{a}^{(12)}_{65}
$$
...
On the surface, it might seem that calculating the accmumulated fund value is simple. However, there are two complications:

* Salary increases every period
* Each contribution gets accumulated by a different factor
