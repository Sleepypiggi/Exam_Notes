...
The expression for profit above is from a **yearly perspective**. It implicitly assumes that the policyholder has **already survived till that period**, thus only the probabilities of dying in *that* period are considered.
