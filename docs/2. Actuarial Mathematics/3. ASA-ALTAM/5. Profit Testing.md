...
* Consider the outgo from **ALL possible benefits**
* **Survival probability** should be for ALL decrements
...
\begin{aligned}
    ...
    \text{PR}^{(i)}_{t} &= \text{NCF}_{t} - [p^{(\tau)}_{x} \cdot {}_{t}V - {}_{t-1}V \cdot (1+i)]
...
!!! Note
    ...
    Remember that multi-decrement models are simply a variant of multi-state models. Thus, the same methods apply.
