...
One common type of question based on NPV is to **estimate the future NPV** of the policy given the current NPV. Consider the cashflows of the following two NPVs:
...
There is a relationship between profits and reserves.
...
However, in order to prevent insolvency, the insurer **MUST set up a reserve** in the earlier years which can be used to cover the shortfall in premium income in the later years to pay for benefits.
...
Thus, if the reserves **decrease across all periods** (EG. Due to an Increase of Interest Rates):
...
Generally speaking, due to the **time value of money**, the uplift from the earlier periods is stronger, resulting in increase to NPV.
...
All the above calculations are known as the **Per Policy** gains - if the actual number of policies are given, then each gain needs to be multiplied by the **STARTING NOP** to determine the total gain made by the insurer.

!!! Warning

    It is a common mistake to use the ending NOP because the "intuition" is that the gain/loss is made on the remaining policies.
