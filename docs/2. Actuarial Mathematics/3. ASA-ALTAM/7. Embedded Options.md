...
!!! Note
    ...
    Under a risk neutral basis, it assumes that investors do NOT take into account risk. Thus, assets will be **priced based on the risk free rate**, rather than on risk premiums.
...
\begin{aligned}
    ...
    &= \text{Portfolio Before Rebalancing}_{t} - \text{Expected Portfolio Required}_{t} \\
    \\
    \text{Portfolio Before Rebalancing}_{t}
    &= \text{Stock Value}_{t-1} \cdot (1+g) + \text{Bond Value}_{t-1} \cdot (1+r) \\
    \\
    \text{Expected Required Portfolio}_{t}
    &= p_{x+t-1} \cdot \pi(t)
\end{aligned}
$$
