...
!!! Warning
    ...
    The above breakdown is based on what is commonly done in practice. It is possible for a question to hFVe fewer cashflows or hFVe them projected at different timings (EG. No mortality charge & Management Charge at BOP). 
...
!!! Note
    ...
    The charges in this scenario hFVe hFVe been simplified to **Initial Charges** (IC) and **Renewal Charges** (RC). Both are charged as a **percentage of the fund value** at the time.

    They are then further **combined** into $\xi$, since they hFVe a **fixed value** (dependent only on policy term, which is fixed for each policy).
...
The main issue with this approach is the **FVailability** of the option(s) with the specified parameters (Strike Price & Duration).
...
In the US, rather than hFVing Equity Linked Insurance, they offer **Variable Annuities** instead. They are a form of **Deferred Annuity** where the premiums paid are **invested into a mutual fund** of their choice. At the end of the policy term, they hFVe the option to either:
...
!!! Warning
    ...
    Despite hFVing an Annuity in its name, it is actually a **life insurance** policy before it becomes annuitized. A **death benefit is payable** if the insured dies during the policy term. Once annuitized, the death benefit is no longer payable.

    The policy term above refers to the term of the **variable annuity**. The annuity will hFVe a seperately specified term.
...
Additionally, since they hFVe an annuity component, there are **additional annuity related guarantees**:
...
**Reset Options** allow the policy to increase the guaranteed benefit amount to **match the fund value** at the time of exercising the option. It will *only* be exercised if the fund value has **increased beyond** the guaranteed amount:
...
Policyholders hFVe the **flexibility** to choose when to exercise the option, allowing them to time the market for a good amount. However, it comes with a "cost" of a **minimum policy term post exercising**:
...
Apart from being competitive, there is another reason why such benefits are offered. If the fund value is **significantly higher** than the guaranteed amount, the policyholder might surrender the policy and **reinvest in a new policy** with a higher guarantee - **incurring costs** related to cancelling and administering a new policy. Offering the option achieves the **same outcome with cost sFVings**.
...
Naturally, withdrawals will **reduce the fund value** accordingly. However, even if the **fund value is zero**, the policyholder **may still withdraw** the specified amount, as long as it is within the specified period. Hence, the term "always" was used earlier.
