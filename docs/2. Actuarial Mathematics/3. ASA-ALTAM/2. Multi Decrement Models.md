...
In each of the above cases, the underlying decrement rates are assumed to be the same. The timing of the independent decrements only affects the **split between each decrement**; the resulting dependent probabilities. The **total probabilities** remain the same.
