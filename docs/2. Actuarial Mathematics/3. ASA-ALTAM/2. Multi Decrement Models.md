...
An implicit assumption made so far is that the decrements in question are acting **throughout the year** and are thus **competing throughout the year**. For this section, we assume that decrements follow **Independent UDD** unless otherwise stated:

<!-- Obtained from Coaching Actuaries -->
![Standard_Illustration](Assets/2.%20Multi%20Decrement%20Models.md/Standard_Illustration.png){.center}

However, decrements could instead occur at **exact times**, in which case **no competition** actually takes place, affecting the conversion from independent to dependent rates. Generally speaking, there are three main scenarios to consider:
...
However, it is possible to solve for the probability directly. Consider the two scenarios:
...
!!! Warning
    ...
    It is a common mistake to use the UDD assumption to determine the last term; it is only applicable assuming the policyholder is at the **start of the year**.

    The reason is because it does not account for the fact that the policyholder has already **survived till time** $s$. Thus, the true probability can be derived using **conditional probability**:
...
Another variation of this combination...
...
!!! Warning
    ...
    Not all benefits may be a lump sum payment. It is possible to receive a fixed annuity as a benefit (EG. Receiving 5 years of payments upon diagnosis of a disability).
