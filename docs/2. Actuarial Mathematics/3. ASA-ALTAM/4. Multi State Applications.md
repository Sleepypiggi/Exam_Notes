...
!!! Warning
    ...
    * ${}_{t}q^{1}_{x:y}$: Probability that $x$ dies within $t$ years; $y$ can die before OR after $t$
    ...
    The confusion comes about because **both are a situation where $X$ dies before $Y$** but the key difference is **whether or not $Y$ dies** within $t$. Interestingly, the difference of the two is the probability that $y$ dies second within $t$ years:
