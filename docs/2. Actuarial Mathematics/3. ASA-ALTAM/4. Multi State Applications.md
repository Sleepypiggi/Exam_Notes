...
Recall that the likelihood function for a single observation is the **PDF of the observation**:
...
Most questions assume that the **Force of Transition is constant**. Thus, the age of the policyholder does not matter, allowing the above to be expanded to:

$$
\begin{aligned}
    L(\theta \mid x)
    &= {}_{t}p^{\bar{ii}}_{x} \cdot \mu^{ij}_{x+t} \\
    &= e^{-t \cdot \sum \mu^{i(\tau)}_{x+t}} \cdot \mu^{ij}_{x}
\end{aligned}
$$
...
\begin{aligned}
    L(\theta)
    &= \prod L(\theta \mid x) \\
    &= e^{-T_{i} \cdot \sum \mu^{i(\tau)}_{x+t}} \cdot (\mu^{ij}_{x})^{D_{ij}} \cdot (\mu^{ik}_{x})^{D_{ik}}
    \cdot e^{-T_{j} \cdot \sum \mu^{j(\tau)}_{x+t}} \cdot (\mu^{jk}_{x})^{D_{jk}} \cdot (\mu^{ji}_{x})^{D_{ji}}
\end{aligned}
$$

The likelihood function can always be simplified into the above, where:

* $T_{i}$: Total **Amount of time** spent in state $i$ for ALL individuals
* $D_{ij}$: **Number of transitions** from state $i$ to $j$
...
\begin{aligned}
    \ell(\theta)
    &= \ln L(\theta) \\
    &= -T_{i} \cdot \sum \mu^{i(\tau)}_{x+t} -T_{j} \cdot \sum \mu^{j(\tau)}_{x+t} \\
    &+ D_{ij} \ln \mu^{ij}_{x} + D_{ik} \ln \mu^{ik}_{x} + D_{jk} \ln \mu^{jk}_{x} + D_{ji} \ln \mu^{ji}_{x}
\end{aligned}
$$
...
\begin{aligned}
    ...
    -T_{i} + \frac{D_{ij}}{\hat{\mu}^{ij}_{x}} &= 0 \\
    \\
...
!!! Tip
    ...
    The difficulty is usually calcualating the total amount of time spent. Typically, it is **easier to work backwards** rather than forwards.

    * There are $n$ people in the state at the start of the year
    * 3 individuals transitions **out of the state** at time 0.3
    * 5 individuals transitions **into the state** at time 0.8

    Assume no transitions occurred:

    * The total amount of time spent in the state would be $n$ years. This will be used as the base for adjustments
    * The 3 individuals would have spent a total of 3 years; but since they **transitioned out** at time 0.3, the remaining 0.7 years must be **subtracted from the time spent**
    * The 5 individuals would have spent no time in the state; but since they **transitioned in** at time 0.8, the remaining 0.2 years must be **added to the time spent**

    $$
    \begin{aligned}
        \text{Time Spent} 
        &= \text{Base} \\
        &- \text{Remaining Transitions Out} \\
        &+ \text{Remaining Transitions In} \\    
    \end{aligned}
    $$
...
\begin{aligned}
    ...
    &= [I(\mu^{ij}_{x})]^{-1} \\
    &= E_{X} \left[\frac{\partial^{2}}{\partial (\mu^{ij}_{x})^{2}} \right]^{-1} \\
    &= E_{X} \left[\frac{D^{ij}}{(\hat{\mu}^{ij}_{x})^{2}} \right]^{-1} \\
    &= \left[\frac{D^{ij}}{(\hat{\mu}^{ij}_{x})^{2}} \right]^{-1} \\
    &= \frac{(\hat{\mu}^{ij}_{x})^{2}}{D^{ij}} \\
    &= \frac{\left(\frac{D_{ij}}{T_{i}} \right)^{2}}{D^{ij}} \\
    &= \frac{D_{ij}}{(T_{i})^{2}}
\end{aligned}
$$
...
!!! Warning
    ...
    The expectation is the expectation with regards to random variable $X$. Thus, since none of the components are with respect to $X$, the expectation is simply equivalent to the values themselves.

Using the variance of the estimator, the **Linear Confidence Interval** can be derived:

$$
    \hat{mu}^{ij} \pm Z_{\frac{1+\alpha}{2}} \cdot \sqrt{\text{Var} (\hat{\mu}^{ij}_{x})}
$$
