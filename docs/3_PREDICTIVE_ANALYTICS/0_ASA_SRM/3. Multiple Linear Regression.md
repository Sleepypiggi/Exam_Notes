...
!!! Warning
    ...
    The MLR coefficient interpretation is DIFFERENT from SLR. Thus, the same variable can have **different coefficients under each context**. This does NOT mean that either is wrong - they are simply on a different basis. 
...
The degree of freedom from any model fitting software typically refers to the **regression degrees of freedom**:

<!-- Obtained from SOA PA Past Year Paper -->
![DEGREE_OF_FREEDOM_R](Assets/3.%20Multiple%20Linear%20Regression.md/DEGREE_OF_FREEDOM_R.png){.center}
...
This is still considered a **Linear Model** because the dependent variable is a **Linear Combination** of the independent variables; the independent variables themselves can be non-linear. The model can still include other independent variables, regardless of their powers; mix of polynomial and non-polynomials are possible.

!!! Note

    Typically, if there is a known relationship between $Y$ and the k-th order of the independent variable, **ALL orders from $1$ up to $k$** are added for the variable. The reason is because we are interested in the way that $Y$ relates to $X$ as a whole. Keeping **all powers** allows the relationship to better expressed and hence have a better fit.
    
    However, the choice of $k$ **may not be clear**, especially for higher powers where the relationship is not easily visualized. As with most other model components, an optimal order can be found via **cross validation**.
...
So far, we have implicitly assumed that all independent variables are quantitative. However, it is possible to to use **Categorical** ones as well. They are typically represented using a **Dummy Variables**, which are can only take on **1 (Yes) or 0 (No)**.
...
!!! Warning
    ...
    Given the above, the **choice of reference category matters** as it will result in a different model altogether due to the different baseline. For the purposes of this exam, it is assumed that the method will **automatically binarize** any categorical variables prior to running.

    Due to the hierarchical principle, even if one of the levels does not have a statistically significant, the method will still retain all the levels in the model.
...
!!! Warning
    ...
    Interaction variables add predictive power but are **not easy to interpret**.
...
A key problem with MLR is that the addition of more variables makes it **more prone to overfitting** (Low Bias, High Variance). Thus, **regularization methods** help to restrict coefficient estimates closer to 0, by placing a **penalty on the size of coefficients**. This helps to reduce variance at the cost of *slightly* increasing bias.
...
Both methods work by applying a penalty proportional to the mangitude of the coefficients. There is **no clear advantage** between the two; the key difference lies in the **ability to drop predictors**:

<center>

|            **Ridge**             |                    **LASSO**                     |
| :------------------------------: | :----------------------------------------------: |
|         Squared Penalty          |                 Absolute Penalty                 |
| Cannot shrink coefficients to 0  |           Can shrink coefficients to 0           |
|  Able to capture small effects   | Unable to capture small effects (bc shrunk to 0) |
| Effective for large coefficients |      Less effective for large coefficients       |
|      Lower Interpretability      |             Higher Interpretability              |
|        Higher Flexibility        |                Lower Flexibility                 |

</center>

<!-- Obtained from SOA PA Modules -->
![REG_COEFF_SIZE](Assets/3.%20Multiple%20Linear%20Regression.md/REG_COEFF_SIZE.png){.center}
...
!!! Note
    ...
    LASSO performs better when the underlying process is only a function of a few key variables, while Ridge performs better when there are multiple predictors. This is intuitive, given that Ridge cannot drop variables, because simpler models tend to be preferred.
...
* Thus, Ridge tends to have more effective shrinkage with **larger coefficients**
...
An interesting point to note is that LASSO can sometimes be used to identify **variable importance**. Since there is a penalty on the absolute variable coefficients, the algorithm might set all **non-important coefficients to 0** to ensure that an especially important variable has a **large coefficient**.

### **Elastic Net (PA Only)**

Elastic Net is an approach the combines the Ridge and LASSO. It adds a constraint that is the **weighted average** of the Ridge and LASSO penalty:
...
The shrinkage parameter is STILL present! It is assumed that the same $\lambda$ is used for both the underlying Ridge and LASSO model. Thus, the elastic net can only drop variables if:

* Sufficiently high underlying $\lambda$
...
!!! Warning
    ...
    This means that from looking at just the **coefficients before and after**, it becomes difficult to distinguish if Ridge/Lasso/Elastic Net was used. Although Ridge is the only one that cant drop variables, it could still be an elastic net with small $\alpha$ and/or small $\lambda$.
    
    Thus, it may be better to change the mindset to think of Elastic Net as the general case and that LASSO/Ridge are extreme cases.

Elastic Net allows the key strengths of the underlying method, allowing for a highly versatile regularization method:

* Effectiveness of shrinkage large coefficients  (from Ridge)
* Ability to shrink coefficients to 0 (from LASSO)

### **Scaling**

For regularization methods, the absolute values of the **coefficients are combined together to form the penalty**. Variables with **different scales will naturally also have coefficients with different scales**. For instance, for the same impact, a variable measured in Dollars versus Thousand Dollars will have 1000 times the size, surely dominating the regularization. Thus, there is a need to **scale the variables beforehand** to ensure that the regularization is performed correctly.

This issue is not present in regular OLS because the coefficients will never have interact with one another.
...
More generally, there are are two main methods of transforming predictors:
