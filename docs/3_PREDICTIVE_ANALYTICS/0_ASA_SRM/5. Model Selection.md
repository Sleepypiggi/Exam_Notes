...
!!! Tip
    ...
    Backward selection tends to result in **larger models** than forward selection. This is because it starts with all predictors, thus is able to find **complementing predictors** that work well together, that otherwise **may not have been found individually**.
...
After determining the desired hyperparameter value, the model needs to be **re-run on the WHOLE training data** again with the selected value. The result from the cross validation is **NOT the final model** because it was run only on a subset of the data.

!!! Warning

    Tuning is done on the **TRAINING data**, NOT the test data. Remember that the test data should should only be used ONCE, to produce the FINAL result on completely unseen data.

    If the test data was used to the optimize the model, then
...
The intuition is that the difference in cross validation errors is **not material** within the one standard error. Since **simplicity** is usually the goal, the simplest model (within one SD) is chosen. Naturally, the model chosen this way will be **less flexible**, but that is intentional.

!!! Warning

    Depending on the size of the absolute size of the CV error and the optimal split, one SD above the minimum might lead to unintuitive results - EG. Tree with no splits. In such cases, simply default to the next best model.
