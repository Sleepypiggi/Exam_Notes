...
!!! Warning
    ...
    * MLR model on a Log-transformed variable: $E[\ln (Y \mid X)] = \beta_{0} + \beta_{1} \cdot X_{1}$
    * GLM model using a log-link variable: $\ln[E(Y \mid X)] = \beta_{0} + \beta_{1} \cdot X_{1}$
    
    Apart from modelling different variables, the underlying distribution is also different - MLR is locked with normal while GLM can use any of the LEF. This also has other downstream differences, such as assumptions on the variance etc.
...
* Standard errors should increase
* Aymptotic normality no longer holds; t-distribution should be used instead
...
!!! Tip
    ...
    Logit and Probit are the most popular methods. However, logit is preferred as it can be expressed in a **closed form** and is **easier to interpret**.
...
!!! Tip
    ...
    The exposure term is known as an **Offset**, as it is added to the original equation.
