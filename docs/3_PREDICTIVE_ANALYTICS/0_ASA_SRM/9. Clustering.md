...
!!! Tip
    ...
    The goal of clustering is to separate observations into distinct groups. Generally speaking, this is more easily achieved by choosing variables with larger ranges, as the observations are likely more spread out; more distinct.
...
!!! Tip
    ...
    Similar to PCA, it is possible to plot a "Scree Plot" for K-means and use the **Elbow method** as well - choosing $k$ such that there is a reasonable amount of variance explained.

    The total variance is the cluster variation when there is only ONE cluster ($k=1$). The proportion of variance is explained is thus the **reduction in variance** when increasing to that specific $k$.
