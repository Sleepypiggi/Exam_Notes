...
!!! Tip
    ...
    * Minimum number of observations **before a split**
    * Minimum number of observations **after a split**
...
!!! Note
    ...
    The complexity parameter ($\alpha$) and the other pre-pruning conditions are all the **Hyperparameters** of a decision tree. Given that there are multiple, they need to be **tuned together** to achieve the best results.
...
However, the **key disadvantage** is that it is **not easily interpreted**. A single tree is easily visualized while a “forest” is not. If interpretation is required, it is not recommended to use an ensemble method.
...
Boosting refers to the process of **sequentially training** a series of **shallow trees** on the **resisudal of the previous tree**. The strength of weak trees are “boosted” by **aggregating** them together to form a stronger model.
