...
!!! Warning
    ...
    The implicit assumption made is that the independent variable chosen is **independent from OTHER independent variables**, such that the PDP captures the true relationship between the two. However, in reality, is likely that the chosen variable is **correlated or has an interaction effect** with some other variable.
...
!!! Note
    ...
    The learning rate is also referred to as the **Shrinkage Parameter** as it reduces the influence of each tree on the overall prediction. Typically, a **slow learning rate** is preferred ($\alpha \lt 1$). Consequently, a **large number of trees** are required to achieve effective results. Conversely, if only a small number of trees are fit, then a large shrinkage is preferred.
...
It is important to understand why Random Forests will not overfit with the number of trees but Boosting does:

* Adding a tree to a random forest simply means taking the average of more observations, which does NOT force the overall model closer to the target. However, for boosting, each added tree explains more and more of the target, thus each additional tree brings it **closer to the target**, making it possible to overfit
* Boosting uses the **SAME underlying data** for each step, while Random Forests only allow a random subset of the data to be used

However, that is not to say that one is better than the other:

* **Random Forest**: Preferred when variance is the key concern
* **Boosting**: Preferred when bias is the key concern
