...
The area under the ROC curve is known as the **Area Under Curve** (AUC). The higher the AUC, the more cutoff points there are that provide higher sensitivity and specificity. Thus, AUC can be used as a **basis to compare** across different models. Thus, an **AUC of 1 represents a perfect classifier**.

!!! Tip

    The key advantage is that this allows analysis over a **range of cutoff thresholds**, while classification error is only an evaluation at one particular threshold.

    That said, it still does NOT provide a method of choosing a specific cutoff value.
