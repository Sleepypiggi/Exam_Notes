...
|               To make predictions               | To identify relationships between variables |
...
!!! Note
    ...
    It can be split using sampling methods or based on time, where the training set contains older observations and the test set contains newer ones, to evaluate the models ability to make future predictions.
...
Using only a training set, the model with the best performance would be a **very complex model**. While such a model performs well on the training data, it actually performs very **poorly on unseen test data**, which will yield poor results in actual use. This is the quintissential reason why a test set is used during evaluation.
...
| Unable to capture the signal | Mistakenly captures the noise |
...
\begin{aligned}
    ...
    &= \text{Prop of Positive} \cdot \text{Sensitivity} + \text{Prop of Negative} \cdot \text{Specificity} 
\end{aligned}
$$
...
* **X-axis** is "flipped", where it starts from a **specificity of 1** rather than 0
...
The model will implicitly fit better to the dominant class, making it **difficult to make predictions** for the minority class, which will likely result in high specificity or sensitivity. Thus, it is common to employ one of the following which will allow for more balanced predictions:
...
|        Insufficient Data        |      Abundance of data       |
...
![OVER_UNDER_SAMPLE](Assets/1.%20Statistical%20Learning.md/OVER_UNDER_SAMPLE.png){.center}
...
!!! Warning
    ...
    <!-- Obtained from SOA PA Modules -->
    ![TRAIN_TARGET_LEAKAGE](Assets/1.%20Statistical%20Learning.md/TRAIN_TARGET_LEAKAGE.png){.center}
