...
Skewness is a measure of the **asymmetry** of a distribution. A perfectly symmetrical distribution such as the normal distribution has no skew at all; while a distribution which tends to produce **large values on only one end** are skewed.

<!-- Obtained from Wikipedia -->
![SKEWNESS](Assets/0.%20Review%20of%20Statistical%20Theory.md/SKEWNESS.png){.center}
...
!!! Tip
    ...
    The key takeaway is that we will **never know true sampling distribution**, we only have an estimate of it which **changes from sample to sample**. 
...
<!-- Obtained from Onlinestatbook -->
![QQ_PLOT_TAILS](Assets/0.%20Review%20of%20Statistical%20Theory.md/QQ_PLOT_TAILS.png){.center}
...
The bounds of the confidence interval are derived **based on the sampling distribution**:
