...
Methods such as Partial Least Squares or PCA can be used as well.
...
![CONCAVE_TRANSFORM](Assets/4.%20Linear%20Regression%20Assumptions.md/CONCAVE_TRANSFORM.png){.center}
...
If the source of heteroscedasticity is known, it is possible to use a **Weighted Least Squares** approach to account for it. Weights are assigned to **each observation**, such that the weight is the amount needed to **scale the the variance to a constant amount** for each observation.

The weights influence the extent to which each observation is used to fit the model. It is NOT an additional variable - the coefficients for all existing variables remain the same, but their statistical significance may change due to the change in variance.

The weights are typically set such that it is inversely proportional to the variance. It may also be derived based on another variable or on context specific information. 
...
The above doesnt "fix" the heteroscedasticity, it simply *accounts* for it by using an alternative method. In another way, heteroscedasticity is **NOT inherently problematic**; the problem is applying a method that assumes Homoscedastacity (linear regression) to a dataset with heteroscedasticity.

!!! Note

    This concepts of weights also apply to GLMs.
...
High dimensionality occurs when the model has **too many variables**. Each variable has its own coefficient, allowing the model to **fit to complex patterns** in the training data, fitting to **BOTH the signal and the noise**, resulting in a high variance model.

Another aspect to consider is the number of variables relative to the underlying variables:

* **More features than observations**: No fit to the model
* **Exactly as many as observations**: Perfectly fit to the training data
* **Slightly less features than observations**: Highly overfit

Having more features allows the model to use more information, **seemingly** improving it. However, it comes at the cost of overfitting, which is why this is sometimes referred to as the **Curse of Dimensionality**.
...
!!! Warning
    ...
    Dimensionality in this context is referring to dimensionality of the entire dataset or the model. If used in context of a specific categorical variable, then it is referring to the number of levels that it has.
